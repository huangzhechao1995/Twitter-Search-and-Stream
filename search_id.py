...
result = open('full_brexit_index.csv').read().split('\n')[:-1]
target = []
for i in result:
	target.append(int(i.split(';')[0]))
target = [ str(i) for i in list_of_users]
...
start_date = datetime(2018,9,20,0,0,0) # or you can pick start_date = today
end_date = start_date + timedelta(7,0) # or you can pick another end_date 
earliestTweet=0# or you can pick latest tweet from previous query
latestTweet=1046601499982336000
